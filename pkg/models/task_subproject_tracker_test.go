...
func TestSubprojectTracker(t *testing.T) {
	...
	t.Run("moving a subproject moves its tracker task to the new parent", func(t *testing.T) {
		db.LoadAndAssertFixtures(t)
		s := db.NewSession()
		defer s.Close()

		oldParent := Project{Title: "move old parent"}
		require.NoError(t, oldParent.Create(s, usr))
		newParent := Project{Title: "move new parent"}
		require.NoError(t, newParent.Create(s, usr))
		sub := Project{Title: "move child", ParentProjectID: &oldParent.ID}
		require.NoError(t, sub.Create(s, usr))
		require.NoError(t, s.Commit())
		...
		defer s2.Close()
		tracker := &Task{}
		has, err := s2.Where("tracked_project_id = ?", sub.ID).Get(tracker)
		require.NoError(t, err)
		require.True(t, has)

		moved := Project{ID: sub.ID, Title: sub.Title, ParentProjectID: &newParent.ID}
		require.NoError(t, moved.Update(s2, usr))
		require.NoError(t, s2.Commit())

		// The same task follows the subproject, so anything the user added to it survives.
		db.AssertExists(t, "tasks", map[string]interface{}{
			"id":                 tracker.ID,
			"project_id":         newParent.ID,
			"tracked_project_id": sub.ID,
		}, false)
		db.AssertMissing(t, "tasks", map[string]interface{}{
			"project_id":         oldParent.ID,
			"tracked_project_id": sub.ID,
		})
	})

	t.Run("a moved tracker task gets a free index in its new parent", func(t *testing.T) {
		db.LoadAndAssertFixtures(t)
		s := db.NewSession()
		defer s.Close()

		oldParent := Project{Title: "index old parent"}
		require.NoError(t, oldParent.Create(s, usr))
		newParent := Project{Title: "index new parent"}
		require.NoError(t, newParent.Create(s, usr))
		// Occupies index 1 in the new parent, which the tracker task also has.
		existing := Task{Title: "already there", ProjectID: newParent.ID}
		require.NoError(t, existing.Create(s, usr))
		sub := Project{Title: "index child", ParentProjectID: &oldParent.ID}
		require.NoError(t, sub.Create(s, usr))
		require.NoError(t, s.Commit())
		...
		require.NoError(t, err)
		require.True(t, has)
		require.Equal(t, existing.Index, tracker.Index)
		...
		movedTracker := &Task{}
		has, err = s2.ID(tracker.ID).Get(movedTracker)
		require.NoError(t, err)
		require.True(t, has)
		assert.Equal(t, newParent.ID, movedTracker.ProjectID)
		assert.NotEqual(t, existing.Index, movedTracker.Index)
	})

	t.Run("detaching a subproject to the top level removes its tracker task", func(t *testing.T) {
		db.LoadAndAssertFixtures(t)
		s := db.NewSession()
		defer s.Close()

		parent := Project{Title: "detach parent"}
		require.NoError(t, parent.Create(s, usr))
		sub := Project{Title: "detach child", ParentProjectID: &parent.ID}
		require.NoError(t, sub.Create(s, usr))
		require.NoError(t, s.Commit())
		...
		defer s2.Close()
		topLevel := int64(0)
		detached := Project{ID: sub.ID, Title: sub.Title, ParentProjectID: &topLevel}
		require.NoError(t, detached.Update(s2, usr))
		require.NoError(t, s2.Commit())
	...
	t.Run("making a top-level project a subproject creates its tracker task", func(t *testing.T) {
		db.LoadAndAssertFixtures(t)
		s := db.NewSession()
		defer s.Close()

		parent := Project{Title: "attach parent"}
		require.NoError(t, parent.Create(s, usr))
		child := Project{Title: "attach child"}
		require.NoError(t, child.Create(s, usr))
		require.NoError(t, s.Commit())

		db.AssertMissing(t, "tasks", map[string]interface{}{
			"tracked_project_id": child.ID,
		})

		s2 := db.NewSession()
		defer s2.Close()
		attached := Project{ID: child.ID, Title: child.Title, ParentProjectID: &parent.ID}
		require.NoError(t, attached.Update(s2, usr))
		require.NoError(t, s2.Commit())

		db.AssertExists(t, "tasks", map[string]interface{}{
			"project_id":         parent.ID,
			"tracked_project_id": child.ID,
			"title":              "attach child Subproject",
		}, false)
	})

	t.Run("a moved tracker task keeps tracking its subproject's progress", func(t *testing.T) {
		db.LoadAndAssertFixtures(t)
		s := db.NewSession()
		defer s.Close()

		oldParent := Project{Title: "sync old parent"}
		require.NoError(t, oldParent.Create(s, usr))
		newParent := Project{Title: "sync new parent"}
		require.NoError(t, newParent.Create(s, usr))
		sub := Project{Title: "sync child", ParentProjectID: &oldParent.ID}
		require.NoError(t, sub.Create(s, usr))
		require.NoError(t, s.Commit())
		...
		s3 := db.NewSession()
		defer s3.Close()
		first := Task{Title: "sync one", ProjectID: sub.ID}
		require.NoError(t, first.Create(s3, usr))
		second := Task{Title: "sync two", ProjectID: sub.ID, Done: true}
		require.NoError(t, second.Create(s3, usr))
